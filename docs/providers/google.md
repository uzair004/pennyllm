...
import type { GoogleProviderConfig } from 'llm-router/types';
...
- **Gemini 2.5 Flash thinking mode:** May return empty `result.text` and undefined usage fields. The llm-router middleware guards against this with `Number(x) || 0`.
- **Enforcement:** Google returns HTTP 429 (`RESOURCE_EXHAUSTED`) with a `retry-after` header. The llm-router retry proxy handles this automatically.
