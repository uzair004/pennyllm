...
| 9   | All new error classes extend PennyLLMError and serialize via toJSON()                | VERIFIED | All 3 classes: `extends PennyLLMError` (which has `toJSON()` at line 41 of base.ts). AuthError line 6, ProviderError line 24, NetworkError line 6   |
...
| 18  | Final error is PennyLLMError subclass (not APICallError) preventing AI SDK double-retry     | VERIFIED | All 4 `throw` statements in retry-proxy.ts (lines 207, 224, 335, 352) throw `buildFinalError()` which returns ProviderError extends PennyLLMError. Zero `throw APICallError` found. |
...
| --------------------------------- | ------------------------------------------------------------------------------ | -------- | --------------------------------------------------------------------------------------------------------------------------------- |
| `src/errors/auth-error.ts`        | AuthError class for 401/403 failures                                           | VERIFIED | 26 lines, class AuthError extends PennyLLMError, code AUTH_FAILED, actionable suggestion                                          |
| `src/errors/provider-error.ts`    | ProviderError class with type discriminator and attempts array                 | VERIFIED | 52 lines, class ProviderError extends PennyLLMError, errorType/attempts/modelId fields, switch-based suggestions                  |
| `src/errors/network-error.ts`     | NetworkError class for connection failures                                     | VERIFIED | 21 lines, class NetworkError extends PennyLLMError, code NETWORK_ERROR, actionable suggestion                                     |
...
| `src/errors/provider-error.ts`    | `src/errors/base.ts`               | extends PennyLLMError                           | VERIFIED | Line 1: `import { PennyLLMError }`, line 24: `extends PennyLLMError`                                                                              |
