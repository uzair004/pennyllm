...
| 4   | close() clears data and marks backend closed; subsequent calls throw                                        | ✓ VERIFIED | MemoryStorage.ts lines 184-188: Sets `this.closed = true` and calls `this.data.clear()`. Lines 58-61: `ensureOpen()` throws `PennyLLMError` with code 'STORAGE_CLOSED' when closed. Contract tests verify (storage.contract.ts lines 142-164) |
...
- Proper error handling with PennyLLMError
