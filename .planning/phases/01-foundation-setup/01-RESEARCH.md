...
│   │   ├── base.ts             # LLMRouterError base class
...
export class RouterEventEmitter extends EventEmitter {
  on<K extends keyof RouterEvents>(
    event: K,
    listener: (...args: RouterEvents[K]) => void
  ): this {
...
const debug = Debug('llm-router:config')
...
// Usage by consumers:
// DEBUG=llm-router:* npm start          (all namespaces)
// DEBUG=llm-router:config npm start     (only config)
// DEBUG=llm-router:config,llm-router:selection npm start  (multiple)
```
...
// Usage:
import { Strategy } from 'llm-router'
...
```typescript
const debug = Debug('llm-router:selection')
